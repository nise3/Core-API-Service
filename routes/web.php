...
$router->post('/sso-authorize-code-grant', function (\Illuminate\Http\Request $request) {
    ...
    $basicAuth = 'Basic ' . base64_encode('FhVqwNp6Q6FV1H8KuuLsh5REQysa:GfrDpy904LjaWNmn7aSwEA1qyEQa');
    $postUrl = 'https://bus-staging.softbdltd.com/oauth2/token?grant_type=authorization_code&code=' . $request->input('code') . '&redirect_uri=' . urlencode($request->input('redirect_uri'));
    if (config("nise3.is_dev_mode")) {
        $postUrl = 'https://192.168.13.206:9448/oauth2/token?grant_type=authorization_code&code=' . $request->input('code') . '&redirect_uri=' . urlencode($request->input('redirect_uri'));
        $basicAuth = 'Basic ' . base64_encode('FoYnKgvzdAwUGgyfDjj4torgkYYa:pASKCuZCu_w7erAsWNkCZHMHMv8a');
    }
...
$router->post('/sso-renew-access-token', function (\Illuminate\Http\Request $request) {
    ...
    */
    ...
    $basicAuth = 'Basic ' . base64_encode('FhVqwNp6Q6FV1H8KuuLsh5REQysa:GfrDpy904LjaWNmn7aSwEA1qyEQa');
    $postUrl = "https://bus-staging.softbdltd.com/oauth2/token?grant_type=refresh_token&refresh_token=" . $request->input('refresh_token');
    if (config("nise3.is_dev_mode")) {
        $postUrl = "https://192.168.13.206:9448/oauth2/token?grant_type=refresh_token&refresh_token=" . $request->input('refresh_token');
        $basicAuth = 'Basic ' . base64_encode('FoYnKgvzdAwUGgyfDjj4torgkYYa:pASKCuZCu_w7erAsWNkCZHMHMv8a');
    }
